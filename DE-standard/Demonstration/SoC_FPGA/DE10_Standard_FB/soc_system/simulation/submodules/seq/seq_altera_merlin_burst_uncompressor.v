...
// $Id: //acds/rel/11.1/ip/merlin/seq_altera_merlin_slave_agent/seq_altera_merlin_burst_uncompressor.sv#1 $
// $Revision: #1 $
// $Date: 2011/08/15 $
// $Author: max $
...
module seq_altera_merlin_burst_uncompressor
#(
    parameter ADDR_W      = 16,
...
(
   ...
   always @(posedge clk or negedge reset_n) begin
     if (~reset_n) begin
       burst_uncompress_busy <= 0;		//*** SV-to-V: '0=>0 
       burst_uncompress_byte_counter <= 0;		//*** SV-to-V: '0=>0 
     end
     else begin
       if (source_valid & source_ready & sink_valid) begin
         ...
         if (last_packet_beat) begin
           ...
           if (burst_uncompress_busy) begin
             ...
             // bursts?
             burst_uncompress_busy <= 1;		//*** SV-to-V: '1=>??? FIXED
           end
         end
       end
     end
   ...
   always @(posedge clk or negedge reset_n) begin
     if (~reset_n) begin
       burst_uncompress_address_base <= 0;		//*** SV-to-V: '0=>0 
     end
   ...
   always @(posedge clk or negedge reset_n) begin
     if (~reset_n) begin
       burst_uncompress_address_offset <= 0;		//*** SV-to-V: '0=>0 
     end
     else begin
