...
module seq_altera_avalon_dc_fifo(
...
`ifdef ALTERA_DCFIFO_SYNCHRONIZER_BUNDLE
...
`else
     hmctl_synchronizer	#(
		 .FIFO_ADDR_BITS(ADDR_WIDTH + 1)
      ) write_crosser (
		.clk(out_clk),
		.aclr_n(out_reset_n),
		.cfg_inc_sync(1'b1),
		.cfg_sync_mode(1'b0),					
		.async_input(in_wr_ptr_gray),
		.sync_output(out_wr_ptr_gray)
      );
`endif
...
`else
     hmctl_synchronizer	#(
		...
		.cfg_inc_sync(1'b1),
		.cfg_sync_mode(1'b0),					
		.async_input(out_rd_ptr_gray),
		.sync_output(in_rd_ptr_gray)
      );
